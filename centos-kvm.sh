...
# Banner SSH
echo "## SELAMAT DATANG DI SERVER PREMIUM $hnbaru ## \n" >> /etc/pesan

echo "DENGAN MENGGUNAKAN LAYANAN SSH DARI SERVER INI BERARTI ANDA SETUJU SEGALA KETENTUAN YANG TELAH KAMI BUAT: \n" >> /etc/pesan
echo "1. Tidak diperbolehkan untuk melakukan aktivitas illegal seperti DDoS, Hacking, Phising, Spam, dan Torrent di server ini; \n" >> /etc/pesan
echo "2. Maks login $llimit kali, jika lebih dari itu maka akun otomatis ditendang oleh server; \n" >> /etc/pesan
echo "3. Pengguna setuju jika kami mengetahui atau sistem mendeteksi pelanggaran di akunnya maka akun akan dihapus oleh sistem; \n" >> /etc/pesan
echo '4. Tidak ada tolerasi bagi pengguna yang melakukan pelanggaran; \n' >> /etc/pesan
echo "Server by $namap ( $nhp )\n\n" >> /etc/pesan
echo "=====================================================================================================================================================" >> /etc/pesan
...
update-rc.d nginx on
update-rc.d php-fpm on
...
echo $dname:$dname"@dziu" | chpasswd
echo $dname > pass.txt
echo $dname"@dziu" >> pass.txt
...
chmod +x stunnel-centos

# cron
cd
service crond start
update-rc.d crond on
service crond stop
...
echo "Password : $dname@dziu"  | tee -a log-install.txt
